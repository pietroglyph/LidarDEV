...
import com.spartronics4915.lib.motion.MotionProfileGoal.CompletionBehavior;
...
public class ProfileFollower
{
     ...
     * @param kv
     *        The proportional gain on velocity error (or derivative gain on
     *        position error).
     * @param kffv
     *        The feedforward gain on velocity. Should be 1.0 if the units of
    ...
    /**
     * @return The current goal (null if no goal has been set since the latest
     *         call to reset()).
     */
     ...
     * forward the previous setpoint the next time update() is called. This
     * almost always forces a MotionProfile update,
     ...
     * 
     * @param latest_state
     *        The latest *actual* state, used only for feedback purposes (unless
     *        this is the first iteration or
     *        reset()/resetSetpoint() was just called, in which case this is the
     *        new start state for the profile).
     * @param t
     *        The timestamp for which the setpoint is desired.
     * @return An output that reflects the control output to apply to achieve
     *         the new setpoint.
     */
    public synchronized double update(MotionState latest_state, double t)
    {
        ...
        double acc = mLatestSetpoint.motion_state.acc();
        if(Double.isNaN(acc))
            acc = 0.0;
        double output = mKp * mLatestPosError 
                        + mKv * mLatestVelError
     ...
     * Note that this does not check whether we
     * are anywhere close to the final setpoint, however.
     * 
     * @return True if the final setpoint has been generated for the current
     *         goal.
     */
     ...
     * definition of achievement depends on the goal's
     * completion behavior).
     * 
